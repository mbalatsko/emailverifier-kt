...
class DisposableEmailCheckerTest {
    private class TestDomainsProvider(private val domains: List<String>) : DomainsProvider {
    ...
    @Test
    fun `isDisposable returns true for exact match`() = runTest {
    ...
    @Test
    fun `isDisposable returns true for subdomains of disposable domains`() = runTest {
        val provider = TestDomainsProvider(testDomains)
        val checker = DisposableEmailChecker(provider)
        checker.loadData()
    ...
    @Test
    fun `isDisposable returns false for non-disposable domains`() = runTest {
        val provider = TestDomainsProvider(testDomains)
        val checker = DisposableEmailChecker(provider)
        checker.loadData()
    ...
    @Test
    fun `init companion correctly loads data`() = runTest {
