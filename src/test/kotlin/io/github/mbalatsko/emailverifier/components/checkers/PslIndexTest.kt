...
class PslIndexTest {
    private class TestDomainsProvider(private val rules: List<String>) : DomainsProvider {
        override suspend fun provide(): List<String> = rules
    }


    @Test
    fun `basic registrability with simple suffix rules`() = runTest {
    ...
    @Test
    fun `wildcard rules block second-level but allow deeper domains`() = runTest {
        val rules = listOf("*.ck")
        val psl = PslIndex(TestDomainsProvider(rules))
        psl.build()
    ...
    @Test
    fun `exception rules override suffix rules`() = runTest {
    ...
    @Test
    fun `no match means non-registrable`() = runTest {
